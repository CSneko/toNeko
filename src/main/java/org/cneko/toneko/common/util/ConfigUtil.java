    ...
    public static String DEFAULT_CONFIG = """
            # 是否启用仅服务端安装，启用后客户端可以不用安装该模组，但是会缺失一部分功能（建议在使用此项时将client-language设置为false）
                                             ...
                                             chat:
                                               ...
                                               # 猫娘口癖，通常会加在聊天末尾，可填写翻译键（即自定义语言文件中的键）或文本，例如："misc.toneko.nya"和"喵~"
                                               # Catgirl tone, usually added at the end of the chat, can be filled in with the translation key (that is, the key in the custom language file) or text, such as "misc.toneko.nya" and "Miao~"
                                               tone: "misc.toneko.nya"
