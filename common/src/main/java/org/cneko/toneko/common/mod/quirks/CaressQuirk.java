...
public class CaressQuirk extends ToNekoQuirk{
    ...
    @Override
    public InteractionResult onNekoInteraction(Player owner, Level world, InteractionHand hand, Player nekoPlayer, EntityHitResult hitResult) {
        super.onNekoInteraction(owner, world, hand, nekoPlayer, hitResult);
        if (owner.getMainHandItem().isEmpty()){
            ...
            // 增加互动值
            neko.addXp(owner.getUUID(), CARESS.getInteractionValue());
            // 发送消息文本
            owner.displayClientMessage(translatable("quirk.toneko.caress.use", TextUtil.getPlayerName(nekoPlayer)), true);
            nekoPlayer.displayClientMessage(translatable("quirk.toneko.caress.be_used", TextUtil.getPlayerName(owner)),true);
