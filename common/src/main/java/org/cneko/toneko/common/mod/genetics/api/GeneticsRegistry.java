...
public class GeneticsRegistry {
    // 基因库
    public static final Map<Identifier, Allele> ALLELES = new HashMap<>();

    // 基因座库 (这里就是存储 Locus 的地方)
    public static final Map<Identifier, Locus> LOCI = new HashMap<>();

    // 野生基因库： 基因座ID -> 该坑位可能出现的基因及其权重
    public static final Map<Identifier, List<WeightedAllele>> WILD_POOLS = new HashMap<>();
    ...
    // 核型ID -> 核型（数据包引用）
    public static final Map<Identifier, SpeciesKaryotype> KARYOTYPES_BY_ID = new HashMap<>();

    // ================== 动态数据追踪（数据包加载/重载用） ==================

    /** 由数据包添加的等位基因 ID 集合 */
    public static final Set<Identifier> DYNAMIC_ALLELES = new HashSet<>();
    /** 由数据包添加的基因座 ID 集合 */
    public static final Set<Identifier> DYNAMIC_LOCI = new HashSet<>();
    /** 由数据包添加的野生基因池条目：locusId -> set of alleleIds */
    public static final Map<Identifier, Set<Identifier>> DYNAMIC_WILD_POOLS = new HashMap<>();
     ...
     * 注册核型并设置其 ID（数据包引用用）
     */
    public static void registerKaryotype(Identifier id, Class<? extends LivingEntity> entityClass, SpeciesKaryotype karyotype) {
        karyotype.setId(id);
    ...
    public static void addWildAllele(Identifier locusId, Identifier alleleId, int weight) {
    ...
    public static void clearDynamicData() {
        ...
        for (Map.Entry<Identifier, Set<Identifier>> entry : DYNAMIC_WILD_POOLS.entrySet()) {
            Identifier locusId = entry.getKey();
            Set<Identifier> alleleIds = entry.getValue();
    ...
    public static Allele getAllele(Identifier id) {
        return ALLELES.get(id);
    }

    public static Locus getLocus(Identifier id) {
        return LOCI.get(id);
    }
     ...
     * 根据核型 ID 获取核型
     */
    public static SpeciesKaryotype getKaryotypeById(Identifier id) {
    ...
    public record WeightedAllele(Identifier alleleId, int weight) {}
}