...
public class AIConfigScreen extends Screen {
    ...
    private void buildEntries() {
        ...
        list.addEntry(new StringEntry(leftX, rowW, "ai.max_concurrent_neko",
                ...
                ConfigUtil.CONFIG.getStringOr("ai.max_concurrent_neko", ""), 8));
        ...
        list.addEntry(new StringEntry(leftX, rowW, "ai.summary.count",
                ...
                ConfigUtil.CONFIG.getStringOr("ai.summary.count", ""), 8));
        ...
        list.addEntry(new StringEntry(leftX, rowW, "ai.actions.energy_cost",
                ...
                ConfigUtil.CONFIG.getStringOr("ai.actions.energy_cost", ""), 8));
        list.addEntry(new StringEntry(leftX, rowW, "ai.actions.diary.cooldown",
                ...
                ConfigUtil.CONFIG.getStringOr("ai.actions.diary.cooldown", ""), 8));
        list.addEntry(new StringEntry(leftX, rowW, "ai.actions.diary.max_entries",
                ...
                ConfigUtil.CONFIG.getStringOr("ai.actions.diary.max_entries", ""), 8));
        ...
        list.addEntry(new StringEntry(leftX, rowW, "ai.proactive.interval",
                ...
                ConfigUtil.CONFIG.getStringOr("ai.proactive.interval", ""), 8));
        ...
        list.addEntry(new StringEntry(leftX, rowW, "ai.nekotalk.rounds",
                ...
                ConfigUtil.CONFIG.getStringOr("ai.nekotalk.rounds", ""), 8));
        list.addEntry(new StringEntry(leftX, rowW, "ai.nekotalk.interval",
                ...
                ConfigUtil.CONFIG.getStringOr("ai.nekotalk.interval", ""), 8));
        ...
        list.addEntry(new StringEntry(leftX, rowW, "ai.tts.service",
                Component.translatable("screen.toneko.config.key.ai.tts.service"), ConfigUtil.CONFIG.getStringOr("ai.tts.service", ""), 64));
    ...
    @Override
    public void onClose() {
        if (minecraft != null) Minecraft.getInstance().setScreen(lastScreen);
    }

    @Override
    public void extractRenderState(@NotNull GuiGraphicsExtractor g, int mx, int my, float pt) {
        // 26.x：背景由屏幕抽取管线自动处理，无需手动调用
        g.centeredText(font, getTitle(), width / 2, 12, COLOR_ACCENT);
        // Provider indicator
        if (!providers.isEmpty()) {
            ...
            g.centeredText(font, Component.literal(label),
                    width / 2, 24, 0xFFC0C0C0);
        }
        // Scroll hint
        g.centeredText(font, Component.literal("▼ scroll ▼"),
    ...
    static class AIConfigListWidget extends AbstractContainerWidget {
        ...
        AIConfigListWidget(int x, int y, int w, int h) {
            super(x, y, w, h, Component.empty(), AbstractScrollArea.defaultSettings(9));
            this.w = w; this.h = h;
        }

        void addEntry(RowEntry e) { entries.add(e); }
        void clearEntries() { entries.clear(); }
        public int contentHeight() { return entries.stream().mapToInt(RowEntry::getHeight).sum(); }
        ...
        protected void extractWidgetRenderState(@NotNull GuiGraphicsExtractor g, int mx, int my, float pt) {
            ...
            g.outline(getX() - 1, getY() - 1, w + 2, h + 2, COLOR_ACCENT);
            ...
            for (RowEntry e : entries) {
                if (cy + e.getHeight() > getY() && cy < getY() + h) {
                    e.extractRenderState(g, getX(), cy, getX() + w, mx, my, pt);
                }
                cy += e.getHeight();
            }
            g.disableScissor();
        ...
        public boolean mouseDragged(net.minecraft.client.input.MouseButtonEvent event, double dx, double dy) {
            ...
            // 转发给聚焦的条目（如滑动条拖动）
            for (RowEntry e : entries) {
                if (e.isFocused() && e.rowDragged(mx, my, btn, dx, dy)) return true;
            }
        ...
        public boolean keyPressed(net.minecraft.client.input.KeyEvent ke) {
            for (RowEntry e : entries) {
                if (e.isFocused() && e.keyPressed(ke)) return true;
            }
        ...
        public boolean charTyped(net.minecraft.client.input.CharacterEvent ce) {
            for (RowEntry e : entries) {
                if (e.isFocused() && e.charTyped(ce)) return true;
            }
    ...
    interface RowEntry {
        int getHeight();
        void extractRenderState(GuiGraphicsExtractor g, int lx, int ly, int rx, int mx, int my, float pt);
        /** 行内点击（屏幕层把新事件解包后调用） */
        default void rowClicked(double mx, double my, int btn) {}
        /** 行内拖动 */
        default boolean rowDragged(double mx, double my, int btn, double dx, double dy) { return false; }
        default boolean isFocused() { return false; }
        default void setFocused(boolean f) {}
        default boolean keyPressed(net.minecraft.client.input.KeyEvent ke) { return false; }
        default boolean charTyped(net.minecraft.client.input.CharacterEvent ce) { return false; }
    }
    ...
    static class SectionEntry implements RowEntry {
        ...
        @Override
        public void extractRenderState(GuiGraphicsExtractor g, int lx, int ly, int rx, int mx, int my, float pt) {
            int cx = (lx + rx) / 2;
            g.horizontalLine(cx - 200, cx + 200, ly + 6, COLOR_SECTION);
            g.centeredText(Minecraft.getInstance().font, Component.translatable(key), cx, ly + 8, COLOR_SECTION);
        }
    }
    ...
    static class BoolEntry implements RowEntry {
        ...
        public void extractRenderState(GuiGraphicsExtractor g, int lx, int ly, int rx, int mx, int my, float pt) {
            ...
            String s = val ? "§a✔ " : "§c✘ ";
            g.text(Minecraft.getInstance().font, s + label.getString(), lx + 4, ly + 5, 0xFFFFFF);
        }
        @Override
        public void rowClicked(double mx, double my, int btn) {
            val = !val;
    ...
    static class StringEntry implements RowEntry {
        ...
        public void extractRenderState(GuiGraphicsExtractor g, int lx, int ly, int rx, int mx, int my, float pt) {
            boolean hover = mx >= lx && mx < rx && my >= ly && my < ly + getHeight();
            g.fill(lx, ly, rx, ly + getHeight(), hover ? 0x15FFFFFF : 0);
            g.text(Minecraft.getInstance().font, label, lx + 4, ly + 4, COLOR_ACCENT);
            edit.setX(lx + 4);
            edit.setY(ly + 16);
            edit.extractRenderState(g, mx, my, pt);
        }
        @Override
        public void rowClicked(double mx, double my, int btn) {
            ...
            focused = hit;
            edit.setFocused(hit);
            if (hit) edit.mouseClicked(mev(mx, my, btn), true);
        }
        @Override
        public boolean keyPressed(net.minecraft.client.input.KeyEvent ke) { return focused && edit.keyPressed(ke); }
        @Override
        public boolean charTyped(net.minecraft.client.input.CharacterEvent ce) { return focused && edit.charTyped(ce); }
    }

    /** 把解包后的旧参数重新打包为 MouseButtonEvent，便于转发给原版控件 */
    ...
    static class MultilineEntry implements RowEntry {
        ...
        MultilineEntry(int lx, int rw, String configKey, Component label, String value, int maxLen, int height) {
            this.lx = lx; this.rw = rw; this.configKey = configKey; this.label = label;
            this.height = height;
            this.edit = MultiLineEditBox.builder().setX(0).setY(0).build(
                    Minecraft.getInstance().font, rw - 6, height - 18, Component.empty());
    ...
    static class SliderEntry implements RowEntry {
        ...
        public void extractRenderState(GuiGraphicsExtractor g, int lx, int ly, int rx, int mx, int my, float pt) {
            boolean hover = mx >= lx && mx < rx && my >= ly && my < ly + getHeight();
            g.fill(lx, ly, rx, ly + getHeight(), hover ? 0x15FFFFFF : 0);
            g.text(Minecraft.getInstance().font, label, lx + 4, ly + 3, COLOR_ACCENT);
            slider.setX(lx + 4);
            slider.setY(ly + 14);
            slider.extractRenderState(g, mx, my, pt);
        }
        @Override
        public void rowClicked(double mx, double my, int btn) {
            focused = true;
            slider.mouseClicked(mev(mx, my, btn), true);
        }
        @Override
        public boolean rowDragged(double mx, double my, int btn, double dx, double dy) {
            return focused && slider.mouseDragged(mev(mx, my, btn), dx, dy);
        }
    }
    ...
    class ProviderEntry implements RowEntry {
        ...
        public void extractRenderState(GuiGraphicsExtractor g, int lx, int ly, int rx, int mx, int my, float pt) {
            ...
            String txt = "◀  " + name + "  ▶";
            g.centeredText(Minecraft.getInstance().font, Component.literal(txt),
    ...
    static class VoiceEntry implements RowEntry {
        ...
        public void extractRenderState(GuiGraphicsExtractor g, int lx, int ly, int rx, int mx, int my, float pt) {
            ...
            int midX = this.lx + 4 + Minecraft.getInstance().font.width(
                    net.minecraft.client.resources.language.I18n.get("screen.toneko.ai_config.tts.voice") + ": ");
            g.text(Minecraft.getInstance().font, Component.literal(display), midX, ly + 8, 0xFFFFFF);
